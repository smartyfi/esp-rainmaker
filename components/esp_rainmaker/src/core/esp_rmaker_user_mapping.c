...
static void esp_rmaker_user_mapping_cb(void *priv_data)
{
    ...
    snprintf(publish_topic, sizeof(publish_topic), "channels/%s/messages/%s", node_id, USER_MAPPING_TOPIC_SUFFIX);
    esp_err_t err = esp_rmaker_mqtt_publish(publish_topic, publish_payload, strlen(publish_payload), RMAKER_MQTT_QOS1, &data->mqtt_msg_id,0);
    if (err != ESP_OK) {
